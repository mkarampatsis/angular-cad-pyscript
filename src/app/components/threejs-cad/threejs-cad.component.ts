...
import  cube  from "@jscad/modeling/src/primitives/cube";
// Import required functions
import { drawCommands } from '@jscad/regl-renderer';
...
export class ThreejsCadComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    const geom = cube({ size: 2 });
    const canvas = this.canvasRef.nativeElement;

    // Initialize the renderer
    const renderer = new ReglRenderer({ canvas });
    const commands = drawCommands({ geometries: [geom] });

    renderer.clearColor = [0.9, 0.9, 0.9, 1];
